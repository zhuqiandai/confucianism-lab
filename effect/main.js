...
export var example = pipe(
  ...
  Effect.flatMap(function (a) {
    return Effect.if(flaky(), {
      onTrue: function () {
        return Effect.succeed([a, "success2"]);
      },
      onFalse: function () {
        return Effect.fail(
          BarError({
...
var successExample = Effect.succeed(["1", "2"]);
...
var catchDie = pipe(
  dieExample,
  Effect.catchAllDefect(function (_) {
    return Effect.fail(_);
  }),
  Effect.ignore,
);
var successful = pipe(
  Effect.zip(catchDie, Effect.succeed("recovered")),
...
var getUserWithOrders = pipe(
  Effect.zipRight(getUserInfo, getUserOrders),
  Effect.map(function (orders) {
    return orders;
  }),
);
var getUserAndOrders = pipe(
  Effect.zip(getUserInfo, getUserOrders),
  Effect.map(function (param) {
...
// sandbox
export var sandboxed = pipe(
  dieExample,
  Effect.sandbox,
  Effect.catchSome(function (_errorCause) {
    return Option.some(Effect.succeed(1));
  }),
  Effect.unsandbox,
);
console.log(Effect.runSync(sandboxed));
